...
var UserWhere = struct {
	...
	Password:        whereHelpernull_Bytes{field: "\"users\".\"password\""},
...
func (r *userR) GetExamParticipants() ExamParticipantSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "first_name", "last_name", "email", "password", "email_verified_at", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"first_name", "last_name", "email", "created_at", "updated_at"}
	userColumnsWithDefault    = []string{"id", "password", "email_verified_at"}
...
func (o *User) ExamParticipants(mods ...qm.QueryMod) examParticipantQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"exam_participants\".\"user_id\"=?", o.ID),
	)
...
func (userL) LoadExamParticipants(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`exam_participants.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddExamParticipantsG(ctx context.Context, insert bool, related ...*ExamParticipant) error {
...
// Appends related to o.R.ExamParticipants.
// Sets related.R.User appropriately.
func (o *User) AddExamParticipants(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ExamParticipant) error {
	var err error
...
func (s UserSlice) LoadExamParticipantsByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadExamParticipantsByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s UserSlice) LoadExamParticipantsByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
...
func (s UserSlice) GetLoadedExamParticipants() ExamParticipantSlice {
...
// LoadTokensByPage performs eager loading of values by page. This is for a 1-M or N-M relationship.
func (s UserSlice) LoadTokensByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadTokensByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s UserSlice) LoadTokensByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*User](s, pageSize) {
		if err := chunk[0].L.LoadTokens(ctx, e, false, &chunk, queryMods(mods)); err != nil {
			return err
		}
	}
	return nil
...
func (s UserSlice) GetLoadedTokens() TokenSlice {
	result := make(TokenSlice, 0, len(s)*2)
	for _, item := range s {
		if item.R == nil || item.R.Tokens == nil {
			continue
		}
		result = append(result, item.R.Tokens...)
	}
	return result
}
